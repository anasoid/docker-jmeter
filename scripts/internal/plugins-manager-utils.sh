...
if [[ "$WITH_BASE_PLUGINS" == "false" ]]; then
   if [[ ! -z "$JMETER_PLUGINS_MANAGER_INSTALL_LIST" ]] || [[ "$JMETER_PLUGIN_MANAGER_INSTALL_FOR_JMX" == "true" ]]; then
...
#JMETER_PLUGIN_MANAGER_INSTALL_FOR_JMX
if [[ "$JMETER_PLUGIN_MANAGER_INSTALL_FOR_JMX" == "true" ]]; then
   echo "Installing plugins for JMX"
   $PluginsManagerCMD install-for-jmx $JMETER_JMX_FINAL
else
   echo "Skip install JMETER_PLUGIN_MANAGER_INSTALL_FOR_JMX"
fi
